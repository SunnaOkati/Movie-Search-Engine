...
public class RBTree<T extends Comparable<T>> {
	...
	private void insertRecurse(Node<T> root, Node<T> x) {
		int cmp = root.movie.getId() - x.movie.getId();
		
		if (cmp > 0) {
			if (root.left.movie == null) {
				root.left = x;
				x.parent = root;
			} else {
	...
	private void insert(Node<T> x) {
		...
		if (root == null) {
			...
			if(search(x.movie.getId()) != null) return;
			insertRecurse(root, x);
		}

		// Fix tree
		while (x.movie.getId() != root.movie.getId() && x.parent.colour == Colour.RED) {
			...
			if (uncle.colour == Colour.RED) {
				...
				if (x.movie.getId() == (left ? x.parent.right.movie.getId() : x.parent.left.movie.getId())) {
    ...
    */
	public void rotateLeft(Node<T> x) {
		...
		if (x.parent != null) {
			...
			if (x.parent.left.movie.getId() == x.movie.getId()){
    ...
    */
	public void rotateRight(Node<T> x) {
		...
		if (x.parent != null) {
			// Determine whether this node is the left or right child of its parent
			if (x.parent.right.movie.getId() == x.movie.getId()) {
	...
	private String preOrder(Node<T> tree) {
		if (tree != null && tree.movie != null) {
			String leftStr = preOrder(tree.left);
			String rightStr = preOrder(tree.right);
			return tree.movie + (leftStr.isEmpty() ? leftStr : " " + leftStr)
	 ...
	 * @return
	 */
	private Node<T> find(Node<T> x, int v) {
		if (x.movie == null)
			return null;

		int cmp = v - (x.movie.getId());
		if (cmp < 0)
